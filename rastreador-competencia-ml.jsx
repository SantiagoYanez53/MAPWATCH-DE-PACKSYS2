...
function App() {
  ...
  async function handleExcelUpload(e) {
    ...
    try {
      ...
      showToast("El catálogo se cargó pero no se pudo guardar");
    }
    e.target.value = "";
  }

  function openInML(title) {
    window.open(mlSearchUrl(title), "_blank", "noopener,noreferrer");
  }
  ...
  async function handlePaste() {
    try {
      ...
      showToast("No se pudo leer el portapapeles (revisa permisos del navegador)");
    }
  }
  ...
  async function saveEvidence() {
    if (!selectedProduct) {
      showToast("Selecciona un producto del catálogo primero");
      return;
    }
    if (!form.compTitle || !form.compPrice) {
      showToast("Falta el título o el precio de la competencia");
      return;
    }
    ...
    const record = {
      ...
      ownTitle: selectedProduct.title,
      ownPrice: selectedProduct.price,
      ownLink: selectedProduct.link,
      compTitle: form.compTitle,
      compPrice: form.compPrice,
      compSeller: form.compSeller,
      compLink: form.compLink,
  ...
  function exportExcel() {
    if (evidences.length === 0) {
      showToast("No hay evidencias para exportar");
      return;
    }
    const rows = evidences.map((ev) => ({
      SKU: ev.sku,
      "Producto propio": ev.ownTitle,
      "Precio propio": ev.ownPrice,
      "Link propio": ev.ownLink,
      "Producto competencia": ev.compTitle,
      "Precio competencia": ev.compPrice,
      "Vendedor competencia": ev.compSeller,
      "Link competencia": ev.compLink,
    ...
    a.href = url;
    a.download = `reporte-competencia-${new Date()
      .toISOString()
      .slice(0, 10)}.xlsx`;
    a.click();
      ...
      <main style={styles.main}>
            ...
            <div style={styles.rowBetween}>
              <div>
                ...
                <p style={styles.muted}>
                  Sube tu Excel con columnas SKU, Título, Precio y Link. Se
                  guarda en este navegador para futuras visitas.
                </p>
              </div>
            ...
            {loadingCatalog ? (
              ...
              <div style={styles.emptyState}>
                ...
                <p style={styles.mutedSmall}>
                  Sube tu Excel para empezar a comparar precios contra la
                  competencia.
                </p>
              </div>
            ) : (
                      ...
                      <tr key={p.id} style={styles.tr}>
                        ...
                        <td style={{ ...styles.td, textAlign: "right" }}>
                          ...
                          <button
                            ...
                            onClick={() => startEvidence(p)}
                            title="Registrar evidencia de competencia"
                          >
          ...
          <section style={styles.twoCol}>
            <div style={styles.card}>
              ...
              {selectedProduct ? (
                <div>
                  <p style={styles.productLine}>
                    <Tag size={14} color="#C9A227" /> {selectedProduct.sku}
                  </p>
                  ...
                  <button
                    style={styles.smallBtnGhost}
                    onClick={() => openInML(selectedProduct.title)}
                  >
                ...
                <div>
                  <p style={styles.muted}>
                    No has seleccionado ningún producto. Ve al catálogo y
                    presiona "Registrar" en el producto que quieras comparar.
                  </p>
                  <button
                    style={styles.smallBtnGold}
              ...
              <div style={styles.hint}>
                ...
                <ol style={styles.hintList}>
                  ...
                  <li>Da clic en el marcador "Capturar ML" de tu navegador (ver pestaña Marcador de navegador).</li>
            ...
            <div style={styles.card}>
              ...
              <button style={styles.pasteBtn} onClick={handlePaste}>
                <ClipboardPaste size={16} /> Pegar del portapapeles
              </button>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, compTitle: e.target.value }))
                }
                placeholder="Ej. Guitarra Eléctrica Fender Squier Debut Negra"
              />
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, compPrice: e.target.value }))
                }
                placeholder="Ej. $4,999"
              />

              <label style={styles.label}>Vendedor</label>
              <input
                style={styles.input}
                value={form.compSeller}
                onChange={(e) =>
                  setForm((f) => ({ ...f, compSeller: e.target.value }))
                }
                placeholder="Nombre de la tienda o vendedor"
              />
              ...
              <input
                ...
                value={form.compLink}
                onChange={(e) =>
                  setForm((f) => ({ ...f, compLink: e.target.value }))
                }
              ...
              <label style={styles.imageDrop}>
                ...
                <span>
                  {form.imageData ? "Cambiar imagen" : "Adjuntar captura"}
                </span>
                <input
                  ref={imageInputRef}
                  type="file"
              ...
              {form.imageData && (
                <img src={form.imageData} alt="evidencia" style={styles.preview} />
              )}

              <button style={styles.saveBtn} onClick={saveEvidence}>
                <Save size={16} /> Guardar evidencia
              </button>
            </div>
          </section>
        )}
          ...
          <section>
            <div style={styles.rowBetween}>
              <div>
                ...
                <p style={styles.muted}>
                  Registro de publicaciones de la competencia capturadas.
                </p>
              </div>
              <button style={styles.uploadBtn} onClick={exportExcel}>
            ...
            {loadingEvidences ? (
              ...
              <div style={styles.emptyState}>
                <p>Todavía no hay evidencias guardadas.</p>
              </div>
            ) : (
              <div style={styles.cardsGrid}>
                {evidences.map((ev) => (
                  <div key={ev.id} style={styles.evCard}>
                    {ev.imageData ? (
                      <img src={ev.imageData} alt="" style={styles.evThumb} />
                    ) : (
                      <div style={styles.evThumbPlaceholder}>
                        <ImageIcon size={22} color="#5c6478" />
                      </div>
                    )}
                    <div style={styles.evBody}>
                      ...
                      <p style={styles.evSeller}>
                        <Store size={12} /> {ev.compSeller || "Vendedor no especificado"}
                      </p>
                      {ev.compLink && (
                        <a
                          href={ev.compLink}
                          ...
                          style={styles.evLink}
                        >
                          <Link2 size={12} /> Ver publicación
                        </a>
                      )}
                      <p style={styles.evDate}>
        ...
        {tab === "marcador" && (
          <section style={styles.card}>
            ...
            <p style={styles.muted}>
              Arrastra el siguiente botón a tu barra de marcadores. Cuando
              estés en una publicación de Mercado Libre, da clic en él para
              copiar automáticamente el título, precio, vendedor y link al
              portapapeles.
            </p>
            <a
            ...
            <div style={styles.hint}>
              <p style={styles.hintTitle}>Instrucciones</p>
              <ol style={styles.hintList}>
                <li>Muestra tu barra de marcadores (Ctrl+Shift+B en Chrome).</li>
                <li>Arrastra el botón de arriba ("📌 Capturar ML") hacia esa barra.</li>
                <li>Entra a Mercado Libre y abre la publicación de la competencia.</li>
                <li>Da clic en el marcador: los datos se copian solos al portapapeles.</li>
                <li>Vuelve a esta app y usa "Pegar del portapapeles" en la pestaña Registrar evidencia.</li>
              </ol>
              <p style={styles.mutedSmall}>
                Nota: Mercado Libre cambia el diseño de sus páginas de vez en
                cuando; si el marcador deja de detectar algún dato, siempre
                puedes copiarlo a mano y pegarlo en el formulario.
              </p>
            </div>
          </section>
        )}
